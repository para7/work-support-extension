...
- 作業ブロック、ホワイトリスト編集以外の機能は一切ない（PoCでは作ったけど意図的に消しました）
- ブロックを解除する手段はない

という2点になるでしょう。

2点目について。iOS にはスクリーンタイムという、1日にアプリを起動する出来る時間を制限する機能があります。  
これを使って誘惑をブロックしていた時期もあったのですが、大きな弱点がありました。自分の使っているスマホならパスワードも分かるので、簡単に解除出来てしまうのです。  
ある日を境に「スクリーンタイムを解除する」ことへの抵抗感が無くなり、全く意味をなしていないことに気づき使うのをやめました。  
(こんなカス人間は自分だけかと思ったのですが、全く同じエピソードを聞いたことがあるのでよくあるケースなのかもしれませんね。)

なので、解除機能はそぎ落としました。作業時間を決めたら、覚悟を決めて取り組んでください。  
作業中もホワイトリストの変更は出来るようにしているので、開けなくて本当に困ったらそこで追加するか、拡張自体をアンインストールしてください。

~~もし、私がブロック中にtwitterをホワイトリストに追加して開くような事態が発生した場合は、この機能も消滅しますのでご了承ください。~~
